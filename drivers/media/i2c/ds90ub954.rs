...
use kernel::{
    acpi, c_str,
    device::property::{self, FwNodeReferenceArgs},
...
mod ti954 {
    ...
    pub(crate) const TIMESTAMP_P0_HI: usize = 3;

    pub(crate) const REG_TIMESTAMP_P0_LO: usize = 0x2b;
    pub(crate) const TIMESTAMP_P0_LO: usize = 4;
...
impl i2c::Driver for Ds90ub954 {
    ...
    fn probe(client: &mut i2c::Client, id_info: Option<&Self::IdInfo>) -> Result<Pin<KBox<Self>>> {
        ...
        let Ds90ub954ParseDtReturn {
            ...
            test_pattern,
            continuous_clock,
        } = ds90ub954_parse_dt(dev)
            .inspect_err(|_err| dev_err!(dev, "error parsing device tree\n"))?;

        let regmap = regmap::Regmap::init_i2c(client, &REGMAP_CONFIG)
            .inspect_err(|err| dev_err!(dev, "regmap init failed ({})\n", err.to_errno()))?;

        let serializers = ds90ub953_parse_dt(client)
            .inspect_err(|_err| dev_err!(dev, "error parsing device tree\n"))?;
...
impl Ds90ub954 {
        ...
        for i in 0..self.serializers.len() {
            ...
            let mut init_serializer = || -> Result<()> {
                ...
                for (i, (&slave, &alias)) in ds90ub953
                    ...
                    .as_slice()
                    .iter()
                    .zip(ds90ub953.i2c_alias.as_slice())
                    .enumerate()
                {
    ...
    fn read(&mut self, register: u32) -> Result<u32> {
        self.regmap.read(register).inspect_err(|err| {
            dev_err!(
    ...
    fn write(&mut self, register: u32, value: u32) -> Result<()> {
        self.regmap.write(register, value).inspect_err(|err| {
            dev_err!(
                self.i2c_client.as_ref(),
    ...
    fn read_rx_port(&mut self, rx_port: RxPort, addr: u32) -> Result<u32> {
        ...
        self.read(addr)
            .inspect_err(|err| dev_err!(dev, "error read register (0x{:02x})\n", addr))
    }
    ...
    fn write_rx_port(&mut self, rx_port: RxPort, addr: u32, value: u32) -> Result<()> {
        ...
        self.write(addr, value)
            .inspect_err(|_err| dev_err!(dev, "error writing register (0x{:02x})\n", addr))
    }

    fn init_testpattern(&mut self) -> Result<()> {
        for (reg, val) in DS90UB95X_TP_REG_VAL {
            self.write(reg, val).inspect_err(|_err| {
                dev_info!(
                    self.i2c_client.as_ref(),
...
fn ds90ub954_parse_dt(dev: &kernel::device::Device) -> Result<Ds90ub954ParseDtReturn> {
    ...
    let csi_lane_count = fwnode.property_read::<u32>(c_str!("csi-lane-count")).or(4);
    dev_info!(dev, "csi-lane-count: {csi_lane_count}\n");

    let csi_lane_speed = fwnode
        .property_read::<u32>(c_str!("csi-lane-speed"))
        .or(1600);
    dev_info!(dev, "csi-lane-speed: {csi_lane_speed}\n");

    let test_pattern = fwnode.property_present(c_str!("test-pattern"));
    if test_pattern {
    ...
    let continuous_clock = fwnode.property_present(c_str!("continuous-clock"));
    if continuous_clock {
...
struct Ds90ub953 {
    ...
    i2c_address: u32,
    csi_lane_count: u32,
    i2c_slave: FwNodeReferenceArgs, // array with the i2c slave addresses
    i2c_alias: FwNodeReferenceArgs, // array with the i2c alias addresses
    continuous_clock: bool,
...
fn ds90ub953_parse_dt(i2c_client: &i2c::Client) -> Result<[Option<Ds90ub953>; NUM_SERIALIZER]> {
    ...
    let mut res = core::array::from_fn(|_| None);

    let fwnode = dev.fwnode().ok_or(ENOENT)?;

    let serializers_node = fwnode
        .get_child_by_name(c_str!("serializers"))
        .ok_or(ENOENT)?;

    for (i, serializer) in serializers_node.children().enumerate().take(NUM_SERIALIZER) {
        let get_u32 = |prop, default| {
            let val = serializer.property_read::<u32>(prop).or(default);
            dev_info!(dev, "{prop}: {val}\n");
            val
        };

        dev_info!(dev, "serializer name: {}", serializer.display_name());
        dev_info!(dev, "serializer path: {}", &*serializer);

        let rx_channel = RxPort::from(get_u32(c_str!("rx-channel"), 0), dev);

        let test_pattern = serializer.property_present(c_str!("test-pattern"));
        if test_pattern {
            dev_info!(dev, "test-pattern enabled\n");
        } else {
            dev_info!(dev, "test-pattern disabled\n");
        ...
        let hs_clk_div = match serializer
            .property_read::<u32>(c_str!("hs-clk-div"))
            .or(hs_clk_div_default)
        {
            1 => 0b000,
            2 => 0b001,
            4 => 0b010,
        ...
        let regmap = regmap::Regmap::init_i2c(&i2c_client, &REGMAP_CONFIG).inspect_err(|err| {
            dev_err!(
                dev,
        ...
        let i2c_slave = match serializer.property_get_reference_args(
            c_str!("i2c-slave"),
            property::NArgs::Prop(c_str!("list-cells")),
            0,
        ) {
            Err(_) => {
                dev_info!(dev, "reading i2c-slave addresses failed\n");
                FwNodeReferenceArgs::default()
            }
            Ok(i2c_addresses) => i2c_addresses,
        };
        ...
        for (slave, alias) in i2c_slave.as_slice().iter().zip(i2c_alias.as_slice()) {
            dev_info!(dev, "slave addr: 0x{slave:X}, alias addr: 0x{alias:X}\n",);
        }

        let continuous_clock = serializer.property_present(c_str!("continuous-clock"));
        if continuous_clock {
            dev_info!(dev, "continuous clock enabled\n");
        } else {
            dev_info!(dev, "discontinuous clock used\n");
        ...
        let i2c_pass_through_all = serializer.property_present(c_str!("i2c-pass-through-all"));
...
impl Ds90ub953 {
    fn init(&mut self) -> Result<()> {
        ...
        if self.test_pattern {
            ...
            self.init_testpattern().inspect_err(|_err| {
                dev_info!(
                    dev,
    ...
    fn init_testpattern(&mut self) -> Result<()> {
        ...
        for (reg, val) in DS90UB95X_TP_REG_VAL {
            self.write(reg, val)
                .inspect_err(|_err| dev_info!(dev, "953: enable test pattern failed\n"))?;
        }
...
impl RxPort {
    fn from(value: u32, dev: &kernel::device::Device) -> RxPort {
        match value {
            x if x == RxPort::Zero as u32 => RxPort::Zero,
            x if x == RxPort::One as u32 => RxPort::One,
            x if x == RxPort::Both as u32 => RxPort::Both,
            _ => {
