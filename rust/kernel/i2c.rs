...
use crate::{
    ...
    device::Device,
    device_id::{self, RawDeviceId, RawDeviceIdIndex},
    driver,
    ...
    extern "C" fn probe_callback(client: *mut bindings::i2c_client) -> kernel::ffi::c_int {
        ...
        match T::probe(&mut client, info) {
            Ok(data) => {
                ...
                // `struct i2c_client`.
                unsafe {
                    bindings::i2c_set_clientdata(client.as_raw(), data.into_foreign().cast())
                };
            }
...
impl<T: Driver + 'static> driver::Adapter for Adapter<T> {
    ...
    fn acpi_id_table() -> Option<crate::acpi::IdTable<Self::IdInfo>> {
        T::ACPI_ID_TABLE
    }
}
...
///
///```
/// # use kernel::{acpi, bindings, c_str, i2c, of};
/// #
...
pub trait Driver {
    ...
    /// An optional table of ACPI device ids supported by the driver.
